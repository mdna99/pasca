...
<div class="section-header-back">
    ...
    <a href="{{ route('cp.secondary-menus.show', $parent) }}" class="btn btn-icon"><i class="fas fa-arrow-left"></i></a>
    @else
    <a href="{{ route('cp.secondary-menus.submenus.show', [$parent->parent_id, $parent]) }}" class="btn btn-icon"><i class="fas fa-arrow-left"></i></a>
    @endif
</div>
<h1>Detail Menu</h1>
<div class="section-header-breadcrumb">
    ...
    <div class="breadcrumb-item {{ $loop->last ? 'active' : '' }}"><a href="{{ $breadcrumb->parent_id == 0 ? route('cp.secondary-menus.show', $breadcrumb->id) : route('cp.secondary-menus.submenus.show', [$breadcrumb->parent_id, $breadcrumb->id]) }}">{{ $breadcrumb->title }}</a></div>
    @endforeach
</div>
@endsection
...
<div class="card d-flex" style="height: 100%; background: linear-gradient(to left, rgba(255,255,255,0),
    ...
    <div class="card-wrap">
        <div class="card-body">
            ...
            <a href="{{ route('cp.secondary-menus.submenus.edit', [$parent, $menu]) }}" class="btn btn-sm btn-light" style="position: absolute; right: 10px; top: 10px" data-toggle="tooltip" data-placement="top" title="" data-original-title="Edit">
...
<div class="d-flex justify-content-between align-items-center mb-2">
    ...
    <a href="{{ route('cp.secondary-menus.submenus.create', $menu) }}" class="btn btn-primary"><i class="fas fa-plus"></i> Tambah Submenu</a>
</div>
<div class="row">
    ...
    <div class="col-md-4">
        <div class="article">
            <div class="article-header">
                ...
                <a href="{{ route('cp.secondary-menus.submenus.edit', [$menu, $submenu]) }}" class="btn btn-sm btn-primary" style="position: absolute; top: 10px; right: 45px" data-toggle="tooltip" data-placement="top" title="" data-original-title="Edit"><i class="fa fa-pencil-alt"></i></a>
                <form id="form-action" method="POST" action="{{ route('cp.secondary-menus.submenus.destroy', [$menu, $submenu]) }}" accept-charset="UTF-8">
                    @method('DELETE')
                    @csrf
            ...
            <div class="card-footer bg-whitesmoke px-2">
                <a href="{{ route('cp.secondary-menus.submenus.show', [$menu, $submenu]) }}" class="btn btn-sm btn-warning w-100"><i class="fa fa-eye"></i> Detail</a>
            </div>
        </div>
    </div>
    @empty
...
<div class="d-flex justify-content-between align-items-center mb-2">
    ...
    <a href="{{ route('cp.secondary-menus.posts.create', $menu) }}" class="btn btn-primary"><i class="fas fa-plus"></i> Tambah Post</a>
</div>
<div class="row">
    <div class="col-md-12">
        <div class="card">
                ...
                <table class="table table-striped table-hover">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="table-fit">
                                <form id="form-action" method="POST" action="{{ route('cp.secondary-menus.posts.destroy', [$menu, $post]) }}" accept-charset="UTF-8">
                                    @method('DELETE')
                                    @csrf
                                    ...
                                    <div class="table-links">
                                        ...
                                        <a href="{{ route('cp.secondary-menus.posts.show', [$menu, $post]) }}">Detail</a>
                                        <div class="bullet"></div>
                                        <a href="{{ route('cp.secondary-menus.posts.edit', [$menu, $post]) }}">Edit</a>
                                        <div class="bullet"></div>
